    ...
    try {
      ...
      const { data: company, error: companyError } = (await supabase
        ...
        .insert({
          ...
          slug:     generateSlug(data.companyName) + '-' + Date.now().toString(36),
          plan:     'free' as const,
      ...
      // 2. Registrar usuário no Supabase Auth
      ...
      const message =
        (err as any)?.message?.includes('already registered')
          ? 'Este e-mail já está cadastrado.'
          : (err as any)?.message ?? 'Erro ao criar conta. Tente novamente.'
      toast.error(message)
    }
  }
